...
const checkInputConfirmSignupPassword = document.getElementById(
...
checkInputConfirmSignupPassword.addEventListener(
  "click",
...
function validateChangeMdpForm() {
  ...
  if (passwordOk && validatePasswordOk && PasswordOk && passwordConfirmOk) {
...
function showConfirmNewPassword() {
  if (inputValidationNewPassword.type === "password") {
    inputValidationNewPassword.type = "text";
  } else {
    inputValidationNewPassword.type = "password";
  }
}

//Fonction Inscription en BDD avec le click sur le boutton inscription
function ChangementMdp() {
  // Ici le changement de mot de passe
}
